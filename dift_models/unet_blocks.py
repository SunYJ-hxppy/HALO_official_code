...
class CrossAttnDownBlock3D(nn.Module):
    ...
    def forward(self, hidden_states, t_real, inverse, h_state, attention_t, inds, temb=None, encoder_hidden_states=None, attention_mask=None, inter_frame=False, **kwargs):
        output_states = ()##################################################################
        ...
        for resnet, attn, motion_module in zip(self.resnets, self.attentions, self.motion_modules):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = resnet(hidden_states, temb)##############################################################
                hidden_states = attn(hidden_states, t_real, inverse, h_state=h_state, attention_t=attention_t, inds=inds, encoder_hidden_states=encoder_hidden_states, inter_frame=inter_frame, **kwargs).sample
                hidden_states = motion_module(hidden_states, temb, encoder_hidden_states=encoder_hidden_states) if motion_module is not None else hidden_states
