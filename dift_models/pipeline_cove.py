...
@dataclass
class COVEPipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]

class 

COVEPipeline(DiffusionPipeline):
    r"""
    pipeline for COVE: unleashing diffusion feature correspondence for consistent video editing.
    ...
    def get_inds(self, hidden_states):
        # hidden_states = hidden_states.reshape(2, hidden_states.shape[0]//2, h, h, hidden_states.shape[-1])
        single_batch = hidden_states[0]
        single_batch = single_batch.permute(1, 2, 3, 0)
        F, H, W, d = single_batch.shape
        k_single = 1

        single_batch = single_batch.reshape(-1, d)
        single_batch_normalized = torch.nn.functional.normalize(single_batch, p=2, dim=1).half()
        sim = single_batch_normalized @ single_batch_normalized.t() #[15*64*64, 15*64*64]
        sim = sim.reshape(F, H, W, F, H, W)
        reshaped_sim = sim.view(F* H* W* F, -1)

        value, indices = reshaped_sim.topk(k_single, dim=1)
        ...
        positions = torch.stack((rows, cols), dim=2).reshape(F, H, W, F, k_single, 2)
        mask = mask.reshape(F, H, W, F, k_single)
        
        frame_indices = torch.zeros((F, H* W, F), dtype=torch.long)
        for i in range(F):
            # 创建一个包含 [0, 1, 2, ..., F-1] 的基本序列
            base_sequence = torch.arange(F)
            
            # 将第 i 个元素移动到序列开头
            sequence = torch.cat((base_sequence[i:i+1], base_sequence[:i], base_sequence[i+1:]))
            
            # 将该序列复制 4096 次以构建 frame_indices[i]
            frame_indices[i] = sequence.repeat(H* W, 1)
            
        positions = positions[:, :, :, :, :k_single, :].squeeze(4)

        h_indices = positions[..., 0].reshape(F, H* W, F)  # 高度坐标[15,64,64, 15, n]
        w_indices = positions[..., 1].reshape(F, H* W, F)  # 宽度坐标[15,64,64, 15, n]
        
        return [frame_indices, h_indices, w_indices]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ###################################################################
        # # inds = self.get_inds(self.unet.conv_in(latents))
        # x_inds = torch.load('/cluster/home3/wjs/COVE/every_frame/wolf/x_inds_3.pt').cuda()
        # y_inds = torch.load('/cluster/home3/wjs/COVE/every_frame/wolf/y_inds_3.pt').cuda()
        # t_inds = torch.load('/cluster/home3/wjs/COVE/every_frame/wolf/t_inds_3.pt').cuda()
        # mask = torch.load('/cluster/home3/wjs/COVE/every_frame/wolf/mask.pth').cuda()
        
        # inds = [x_inds, y_inds, t_inds, mask]
        #####################################################################
        ...
        return COVEPipelineOutput(videos=video)
