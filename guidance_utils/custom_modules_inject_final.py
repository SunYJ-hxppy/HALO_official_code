...
class InjectionProcessor:
    ...
    def __init__(self, block_name, store=None, output_path=None):
        ...
        self.inject_feature = False
        self.copy_feature = False
        ...
        self.timestep = None
        self.block_idx = None
        self.heads_to_inject_mask = None
    ...
    def __call__(
        ...
        **cross_attention_kwargs, # Receive kwargs
    ) -> torch.Tensor:
        text_seq_length = encoder_hidden_states.size(1)

        # timestep = cross_attention_kwargs.get("timestep", None)
        # block_idx = cross_attention_kwargs.get("block_idx", None) # Use -1 as a default if not found
        timestep = self.timestep
        block_idx = self.block_idx

        # Convert tensor timestep to integer if necessary
        ...
        if self.inject_kv:
            ...
            value[-1:, self.heads_to_inject_mask, text_seq_length:] = self.value[-1:, self.heads_to_inject_mask, text_seq_length:]
        elif self.copy_kv:
            self.key = key[-1:]
            self.query = query[-1:]
            self.value = value[-1:]
