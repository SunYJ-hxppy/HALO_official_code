...
class ControlledTransformer(CogVideoXTransformer3DModel):
    ...
    def forward(
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            if hasattr(block.attn1, 'processor') and block.attn1.processor is not None:
                # 현재 timestep과 block 인덱스를 processor 객체의 속성으로 직접 설정
                block.attn1.processor.timestep = timestep
                block.attn1.processor.block_idx = i
                
            block_cross_attention_kwargs = {}
            if cross_attention_kwargs is not None:
                # Add the block index here
                block_cross_attention_kwargs = cross_attention_kwargs.copy()
            block_cross_attention_kwargs["block_idx"] = i

            
            # print(f"--- 2. Before calling block {i}: {block_cross_attention_kwargs} ---")
            ...
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states, encoder_hidden_states = block(
                    ...
                    cross_attention_kwargs=block_cross_attention_kwargs,
                )
